...
class Game:
    ...
    def put_ship(self, player, ship, message=None):
        ...
        if not self.validate_input(location=location):
            return self.put_ship(
                player=player,
                ship=ship,
                message=error_input.format(
                    ...
                    BOARD_SIZE))
        ship.col_idx, ship.row_idx = self.get_idx_from_location(location)
        ship.orientation = input('Is it horizontal or vertical? (H)/V '
                                 ).lower().strip()
        if not self.validate_ship_fit(
                ship=ship,
        ...
        if not self.validate_ship_overlap(
                ship=ship,
    ...
    def mark_ship(self, ship, marker, board):
    ...
    def validate_ship_fit(self, ship, board):
    ...
    def validate_ship_overlap(self, ship, board):
    ...
    def mark_result_of_shot(self, player, col_idx, row_idx, value):
    ...
    def validate_shot(self, location, player):
        ...
        if value == EMPTY:
            ...
            ship = self.get_ship_from_location(
                player=player.opponent,
                col_idx=col_idx,
                row_idx=row_idx)
            ship.occup.remove((row_idx, col_idx))
            if len(ship.occup) == 0:
                self.sink_ship(ship=ship, player=player)
    ...
    def shoot(self, player, message=None):
        ...
        return self.validate_shot(location=location, player=player)
        ...
        while True:
            for player in players:
                ...
                if len(player.opponent.ships) == 0:
                    ...
                    print(message)
                    input('{} wins! Press enter to exit the game. '.format(
                        player.name))
                    exit()
                else:
